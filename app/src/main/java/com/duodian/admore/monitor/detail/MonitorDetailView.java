...
/**
 * Created by duodian on 2017/11/3.
 * monitor detail view
 */

public class MonitorDetailView extends View {
    ...
    public MonitorDetailView(Context context) {
        this(context, null);
    }

    public MonitorDetailView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MonitorDetailView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText(String.valueOf(passNumNow), 0, String.valueOf(passNumNow).length(), getWidth() / 2 - rect.width() / 2,
        ...
        canvas.drawText(" / " + String.valueOf(totalNum), 0, (" / " + String.valueOf(totalNum)).length(),
                getWidth() / 2 - rect.width() / 2 + passRect.width(), getHeight() / 2 + rect.height() / 2, paint);

        paint.setTextSize(percentTextSize);
        canvas.drawText(String.valueOf(percentNow + " % "), 0, String.valueOf(percentNow + " % ").length(),
                getWidth() / 2 - percentRect.width() / 2, getHeight() / 2 + percentRect.height() * 3.5f, paint);
    ...
    public void onScale() {
        valueAnimator_scale = ValueAnimator.ofFloat(1f, 0f).setDuration(300);
    ...
    public void onRecover() {
        valueAnimator = ValueAnimator.ofFloat(0, 1f).setDuration(300);
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                float fraction = animation.getAnimatedFraction();
