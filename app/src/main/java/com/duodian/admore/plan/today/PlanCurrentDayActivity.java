...
public class PlanCurrentDayActivity extends BaseActivity implements OnPlanActionListener, View.OnClickListener {

    private static final String TAG = "PlanCurrentDayActivity";
    ...
    @BindView(R.id.linear_spread_start)
    LinearLayout linear_spread_start;

    @BindView(R.id.linear_spread_pause)
    LinearLayout linear_spread_pause;

    @BindView(R.id.linear_spread_stop)
    LinearLayout linear_spread_stop;

    @BindView(R.id.linear_footerView)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_plan_current_day);
    ...
    private void initViews() {
        ...
        textView_title.setText(getResources().getString(R.string.spreadToday));

        linearLayoutManager = new LinearLayoutManager(this);
        recyclerView.setLayoutManager(linearLayoutManager);
        recyclerView.addItemDecoration(new DividerItemDecoration(this, LinearLayoutManager.VERTICAL));
        planKeywordInfoList = new ArrayList<>();
        planListAdapter = new PlanListAdapter(this, planKeywordInfoList, PlanListAdapter.TYPE_TODAY);
        ...
        linear_spread_start.setOnClickListener(this);
        linear_spread_pause.setOnClickListener(this);
        linear_spread_stop.setOnClickListener(this);

        linear_footerView.post(new Runnable() {
            @Override
            public void run() {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    linear_footerView.setOutlineProvider(new ViewOutlineProvider() {
                        @Override
                        public void getOutline(View view, Outline outline) {
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                                outline.setRect(0, 0, linear_footerView.getWidth(), linear_footerView.getHeight());
                            }
                        }
                    });
                }
    ...
    private void getPlans(final int rowNum) {
        ...
        Observable<HttpResult<PlanListInfo>> observable = iServiceApi.todayPlanList(params, System.currentTimeMillis());
                ...
                .subscribe(new Observer<HttpResult<PlanListInfo>>() {
                    ...
                    public void onNext(@NonNull HttpResult<PlanListInfo> planListInfoHttpResult) {
                        ...
                        if (rowNum == 0) {//第一次加载或者重新加载
                            ...
                            planListAdapter.notifyItemRemoved(0);
                            currentListNum = 0;
                        ...
                        for (int i = 0; i < appInfoList.size(); i++) {
                            PlanKeywordInfo planKeywordInfoDate = new PlanKeywordInfo();
                            planKeywordInfoDate.setItemType(PlanKeywordInfo.TYPE_DATE);
                            planKeywordInfoDate.setYmd(appInfoList.get(i).getYmd());
                            planKeywordInfoList.add(planKeywordInfoDate);
                            ...
                            for (PlanKeywordInfo planKeywordInfo1 : appInfoList.get(i).getKeywords()) {
                                ...
                                if (planKeywordInfo1.isOne()) {//同一天
                                    stringBuilder.append(" ").append(planKeywordInfo1.getStartTime()).append(" 至 ").append(planKeywordInfo1.getEndTime());
                                } else {
...
//                        planListAdapter.notifyItemRangeInserted(currentListNum, planKeywordInfoList.size() - currentListNum);
    ...
    private void startPlan(final String keywordId, final int position) {
        ...
        Observable<HttpResult> observable = iServiceApi.todayPlanStart(params, System.currentTimeMillis());
        observable.subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    private void pausePlan(final String keywordId, final int position) {
        ...
        params.put("identifier", Global.userInfo.getIdentifier());
        params.put("keywordId", keywordId);
        Observable<HttpResult> observable = iServiceApi.todayPlanPause(params, System.currentTimeMillis());
        observable.subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<HttpResult>() {
                    @Override
    ...
    public void onClick(View v) {
        int id = v.getId();
        switch (id) {
            case R.id.linear_spread_start:
                break;
            case R.id.linear_spread_pause:
                break;
            case R.id.linear_spread_stop:
                break;
        }
